...
-- select 10000000
-- $$;
create schema if not exists api_svc;
set search_path to api_svc, public;

drop table if exists api_svc.service_point cascade;
drop table if exists institution cascade;
drop table if exists api_svc.app_user cascade;
drop table if exists sp_user cascade;
drop table if exists operator cascade;
drop table if exists api_svc.raido_operator cascade;
drop table if exists auth_request cascade;
drop table if exists authz_request cascade;
drop table if exists api_svc.user_authz_request cascade;
...
comment on constraint unique_name on api_svc.service_point is
  'initially for uniqueness rather than querying, we want to retain the case
  entered by the user';

comment on column api_svc.service_point.search_content is
...
create table api_svc.app_user (
  ...
  service_point_id bigint
    references api_svc.service_point                            not null,
...
create unique index app_user_id_fields_active_key
  on api_svc.app_user(email, client_id, subject)
  where enabled = true;
...
comment on column api_svc.app_user.token_cutoff is
...
comment on column api_svc.app_user.id_provider is
...
comment on column api_svc.app_user.email is
...
comment on table api_svc.raido_operator is
...
create table api_svc.user_authz_request (
  ...
  service_point_id bigint                      not null references api_svc.service_point,
  email            varchar(256)                not null,
  client_id        varchar(256)                not null,
  id_provider      id_provider                 not null,
  subject          varchar(256)                not null,
  responding_user  bigint references api_svc.app_user  null,
  approved_user    bigint references api_svc.app_user  null,
...
alter table api_svc.user_authz_request
  add constraint lowercase_email_constraint
    check (email = lower(email));
comment on column api_svc.user_authz_request.email is
  'Lowercase chars only - db enforced';

create unique index user_authz_request_once_active_key
  on api_svc.user_authz_request(service_point_id, client_id, subject)
  where status = 'REQUESTED';
comment on index user_authz_request_once_active_key is
  'a user can only make one active request per service point';

comment on column api_svc.user_authz_request.responding_user is
  'user that approved or rejected, not set until that happens';

comment on column api_svc.user_authz_request.date_responded is
  'not set until approved or rejected';

comment on column api_svc.user_authz_request.approved_user is
...
insert into api_svc.raido_operator
